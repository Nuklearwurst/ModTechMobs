			...
			if (!target.hitByEntity(origin)) {
				if (target instanceof EntityLivingBase) {
					...
					if(attackDamage > 0) {
						...
						origin.world.playSound(null, origin.posX, origin.posY, origin.posZ, SoundEvents.ENTITY_PLAYER_BURP, SoundCategory.PLAYERS, 1.0F, origin.getRNG().nextFloat() * 0.1F + 0.9F);
							...
							if (data > Settings.TechData.SAFE_TECH_VALUE) {
								...
								if (rand >= 0.8 * data) {
									...
									if (data > TechDataStorage.getInstance().getDangerousPlayerLevel()) { //very nasty
										...
										origin.world.createExplosion(null, origin.posX, origin.posY + 1, origin.posZ, 0.4F, false);
									} else if (data > 0.8 * TechDataStorage.getInstance().getDangerousPlayerLevel()) { //normal effects
										TDEffectHandler.applyRandomEffectOnPlayer(origin, origin.getUniqueID(), itemRand);
										stack.damageItem(10, origin);
									} else { //simple effects
										origin.world.playSound(null, origin.posX, origin.posY, origin.posZ, SoundEvents.ENTITY_ENDERMEN_SCREAM, SoundCategory.PLAYERS, 1.0F, itemRand.nextInt() + 0.5F);
